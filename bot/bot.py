...
⚪ /retry – Пересоздать последний ответ бота
...
2. Назначьте его <b>администратором</b>, чтобы он мог видеть сообщения (все остальные права могут быть ограничены)
3. Красава!

Чтобы получить ответ от бота в чате, укажите его через @ <b>тег</b> или <b>ответьте</b> на его сообщение.
...
async def register_user_if_not_exists(update: Update, context: CallbackContext, user: User):
    ...
    # обратная совместимость для поля n_used_tokens
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я бот <b>ChatGPT</b>, созданный с использованием OpenAI API 🤖\n\n"
...
async def retry_handle(update: Update, context: CallbackContext):
    ...
    db.set_dialog_messages(user_id, dialog_messages, dialog_id=None)  # последнее сообщение было удалено из контекста

    await message_handle(update, context, message=last_dialog_message["user"], use_new_dialog_timeout=False)


async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    # проверка, был ли упомянут бот (для групповых чатов)
    ...
    # удалить упоминание бота (в групповых чатах)
        ...
        if use_new_dialog_timeout:
            if (datetime.now() - db.get_user_attribute(user_id,
                ...
                await update.message.reply_text(
                    f"Начинаю новый диалог из-за тайм-аута (<b>{config.chat_modes[chat_mode]['name']}</b> режим) ✅",
                    parse_mode=ParseMode.HTML)
        db.set_user_attribute(user_id, "last_interaction", datetime.now())

        # в случае ошибки отмены
        n_input_tokens, n_output_tokens = 0, 0
        current_model = db.get_user_attribute(user_id, "current_model")

        try:
            # отправить заполнительное сообщение пользователю
            ...
            if _message is None or len(_message) == 0:
                await update.message.reply_text("🥲 Вы отправили <b>пустое сообщение</b>. Пожалуйста, попробуйте снова!",
                                                parse_mode=ParseMode.HTML)
                return
            ...
            async for gen_item in gen:
                ...
                answer = answer[:4096]  # ограничение телеграма на сообщение

                # обновить только тогда, когда готово 100 новых символов
                ...
                await asyncio.sleep(0.01)  # подождать немного, чтобы избежать затопления
...
async def is_previous_message_not_answered_yet(update: Update, context: CallbackContext):
    ...
    if user_semaphores[user_id].locked():
        ...
        text += "Или вы можете его отменить, используя команду /cancel"
...
async def error_handle(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        message = (
            f"Во время обработки обновления произошло исключение\n"
